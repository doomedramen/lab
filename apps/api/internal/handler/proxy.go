...
// ListProxyHostsByTarget returns proxy hosts targeting a specific IP address.
func (s *ProxyServiceServer) ListProxyHostsByTarget(
	ctx context.Context,
	req *connect.Request[labv1.ListProxyHostsByTargetRequest],
) (*connect.Response[labv1.ListProxyHostsByTargetResponse], error) {
	hosts, total, err := s.proxySvc.ListHostsByTargetIP(ctx, req.Msg.TargetIp)
	if err != nil {
		return nil, connect.NewError(connect.CodeInternal, err)
	}

	return connect.NewResponse(&labv1.ListProxyHostsByTargetResponse{
		ProxyHosts: hosts,
		Total:      total,
	}), nil
}
