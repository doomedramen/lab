...
func VMTemplates() []VMTemplate {
	return []VMTemplate{
		...
		},
		{
			ID:            "rocky-9",
			Name:          "Rocky Linux 9",
			Description:   "Rocky Linux 9.7 - RHEL compatible (Minimal Boot)",
			Icon:          "🎩",
			ISOURLx86_64:  "https://download.rockylinux.org/pub/rocky/9/isos/x86_64/Rocky-9-latest-x86_64-boot.iso",
			ISOURLaarch64: "https://download.rockylinux.org/pub/rocky/9/isos/aarch64/Rocky-9-latest-aarch64-boot.iso",
			ISOName:       "Rocky-9-latest-${arch}-boot.iso",
			CPUCores:      2,
			MemoryGB:      4,
			DiskGB:        20,
		...
		},
		{
			ID:            "almalinux-9",
			Name:          "AlmaLinux 9",
			Description:   "AlmaLinux 9 - RHEL compatible (Minimal Boot)",
			Icon:          "🎩",
			ISOURLx86_64:  "https://repo.almalinux.org/almalinux/9/isos/x86_64/AlmaLinux-9-latest-x86_64-boot.iso",
			ISOURLaarch64: "https://repo.almalinux.org/almalinux/9/isos/aarch64/AlmaLinux-9-latest-aarch64-boot.iso",
			ISOName:       "AlmaLinux-9-latest-${arch}-boot.iso",
			CPUCores:      2,
			MemoryGB:      4,
			DiskGB:        20,
		...
		},
		{
			ID:            "fedora-41",
			Name:          "Fedora 41",
			Description:   "Fedora 41 - Network Install",
			Icon:          "🎩",
			ISOURLx86_64:  "https://download.fedoraproject.org/pub/fedora/linux/releases/41/Server/x86_64/iso/Fedora-Server-netinst-x86_64-41-1.4.iso",
			ISOURLaarch64: "https://download.fedoraproject.org/pub/fedora/linux/releases/41/Server/aarch64/iso/Fedora-Server-netinst-aarch64-41-1.4.iso",
			ISOName:       "Fedora-Server-netinst-${arch}-41-1.4.iso",
			CPUCores:      2,
			MemoryGB:      2,
			DiskGB:        20,
...
// BindPCIDeviceToVFIO binds a PCI device to the vfio-pci driver
func (s *VMService) BindPCIDeviceToVFIO(ctx context.Context, pciAddr string) error {
	if s.pciRepo == nil {
		return errors.New("PCI repository not configured")
	}

	return s.pciRepo.BindDeviceToVFIO(ctx, pciAddr)
}

// UnbindPCIDeviceFromVFIO unbinds a PCI device from vfio-pci and restores original driver
func (s *VMService) UnbindPCIDeviceFromVFIO(ctx context.Context, pciAddr string) error {
	if s.pciRepo == nil {
		return errors.New("PCI repository not configured")
	}

	return s.pciRepo.UnbindDeviceFromVFIO(ctx, pciAddr)
}
