...
// ListStorageContent lists content in a storage pool
func (s *StorageService) ListStorageContent(ctx context.Context, poolID, contentType string) ([]*model.StorageContent, error) {
	...
	if pool == nil {
		return nil, fmt.Errorf("storage pool not found: %s", poolID)
	}

	// List files in the pool directory
	var contents []*model.StorageContent
	...
	if pool.Path != "" {
		...
		for _, entry := range entries {
			...
			if contentType != "" {
				if !matchesContentType(name, contentType) {
					continue
				}
			}
			...
			info, err := entry.Info()
			if err != nil {
				slog.Warn("Failed to get file info", "name", name, "error", err)
				continue
			}
			...
			contents = append(contents, &model.StorageContent{
				ID:         uuid.New().String(),
				PoolID:     poolID,
				Name:       name,
				Type:       contentType,
...
// matchesContentType checks if a filename matches the requested content type
func matchesContentType(filename, contentType string) bool {
	switch contentType {
	case "iso":
	...
	case "backup":
		return strings.HasSuffix(strings.ToLower(filename), ".bak") || strings.HasSuffix(strings.ToLower(filename), ".tar.gz")
	case "template":
...
// detectContentType detects the content type from filename
func detectContentType(filename string) string {
	lower := strings.ToLower(filename)
	if strings.HasSuffix(lower, ".iso") {
		return "iso"
	}
	if strings.HasSuffix(lower, ".qcow2") || strings.HasSuffix(lower, ".raw") || strings.HasSuffix(lower, ".img") {
		return "disk"
	}
	if strings.HasSuffix(lower, ".bak") || strings.HasSuffix(lower, ".tar.gz") {
		return "backup"
	}
...
// getFileFormat returns the file format from filename
func getFileFormat(filename string) string {
	ext := strings.TrimPrefix(strings.ToLower(filename[strings.LastIndex(filename, "."):]), ".")
	if ext == "" {
		return "unknown"
	}
	return ext
