...
// ValidateJWTSecret validates a JWT signing secret for security.
// HS256 requires at least 256 bits (32 bytes) for adequate security.
func ValidateJWTSecret(secret string) ValidationErrors {
	var errs ValidationErrors

	if secret == "" {
		return appendError(errs, "jwt_secret", "is required. Generate a secure random secret with: openssl rand -base64 32")
	}

	// Minimum 32 characters for HS256 (256 bits)
	if len(secret) < 32 {
		errs = appendError(errs, "jwt_secret", "must be at least 32 characters for HS256 security. Generate with: openssl rand -base64 32")
	}

	// Maximum 256 characters (sanity cap)
	if len(secret) > 256 {
		errs = appendError(errs, "jwt_secret", "must be 256 characters or less")
	}

	// Check for leading/trailing whitespace (common copy-paste error)
	trimmed := strings.TrimSpace(secret)
	if trimmed != secret {
		errs = appendError(errs, "jwt_secret", "must not have leading or trailing whitespace")
	}

	// Check for common weak patterns (case-insensitive)
	lowerSecret := strings.ToLower(secret)
	weakPatterns := []string{"secret", "password", "changeme", "jwt", "key", "token", "auth"}
	for _, pattern := range weakPatterns {
		if strings.Contains(lowerSecret, pattern) {
			errs = appendError(errs, "jwt_secret", "contains weak pattern '"+pattern+"'. Use a cryptographically random secret")
			break // Only report one weak pattern error
		}
	}

	// Check for all same character (no entropy)
	if len(secret) > 0 && hasZeroEntropy(secret) {
		errs = appendError(errs, "jwt_secret", "has no entropy (all same character). Use a cryptographically random secret")
	}

	return errs
}

// hasZeroEntropy checks if all characters in the string are the same
func hasZeroEntropy(s string) bool {
	if len(s) == 0 {
		return false
	}
	first := rune(s[0])
