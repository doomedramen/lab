...
func Router(
	...
	// Middleware
	r.Use(appmiddleware.RequestIDWithResponseHeader)
	r.Use(middleware.RealIP)
	r.Use(appmiddleware.Logging)
	r.Use(appmiddleware.CORS)
	r.Use(middleware.Recoverer)
	
	// Audit logging - logs important actions for security compliance
	// Placed after logging but before timeout to capture all requests
	if auditRepo != nil {
		r.Use(appmiddleware.Audit(auditRepo, appmiddleware.DefaultAuditConfig()))
	}
	
	// Request timeout middleware - prevents hanging requests
	// Default 60s timeout, with longer timeouts for backup/snapshot/ISO operations
	r.Use(appmiddleware.Timeout(appmiddleware.DefaultTimeoutConfig()))

	// Request size limiting - protect against oversized request bodies
	// This is applied early to reject large requests before they consume resources
	if maxRequestBodySize > 0 {
		r.Use(appmiddleware.RequestSizeMiddleware(maxRequestBodySize))
	}
	...
	// GitOps service (if available)
	if gitOpsSvc != nil {
		gitOpsHandler := handler.NewGitOpsServiceServer(gitOpsSvc)
		connectMux.Handle(labv1connect.NewGitOpsServiceHandler(gitOpsHandler, connect.WithInterceptors(interceptors...)))
	}
