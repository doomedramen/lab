...
type ContainerRepository struct {
	...
	emulatorPath string // Path to libvirt_lxc emulator
...
// emulatorPath is the path to libvirt_lxc emulator (default: /usr/lib/libvirt/libvirt_lxc).
func NewContainerRepository(client libvirtx.LibvirtClient, rootDir, emulatorPath string) *ContainerRepository {
	if rootDir == "" {
		rootDir = "/var/lib/lxc"
	}
	if emulatorPath == "" {
...
</domain>`, domainName, title, memKiB, vcpus, r.emulatorPath, rootfs, mac)
}
