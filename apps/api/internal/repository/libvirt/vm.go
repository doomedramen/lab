...
func NewVMRepository(client libvirtx.LibvirtClient, cfg *config.Config) *VMRepository {
	return &VMRepository{
		client: client,
		cfg:    cfg,
	}
...
func (r *VMRepository) buildDomainXML(req *model.VMCreateRequest, vmid int, memoryKiB float64) string {
    ...
    <loader readonly='yes' type='pflash'>%s</loader>`, ovmfPath)
			...
			nvramTemplate := ""
			if req.SecureBoot {
				if varsPath := r.cfg.GetOVMFVarsPath(); varsPath != "" {
					nvramTemplate = fmt.Sprintf(" template='%s'", varsPath)
				}
			}
