...
func NewPCIRepository(client libvirtx.LibvirtClient) *PCIRepository {
...
// BindDeviceToVFIO binds a PCI device to the vfio-pci driver
// This requires root privileges and the device must not be in use
func (r *PCIRepository) BindDeviceToVFIO(_ context.Context, pciAddr string) error {
	normalizedAddr := normalizePCIAddress(pciAddr)
	if normalizedAddr == "" {
		return fmt.Errorf("invalid PCI address: %s", pciAddr)
	}

	devicePath := fmt.Sprintf("/sys/bus/pci/devices/%s", normalizedAddr)

	// Check if device exists
	if _, err := os.Stat(devicePath); os.IsNotExist(err) {
		return fmt.Errorf("PCI device %s not found", normalizedAddr)
	}

	// Get current driver
	currentDriver := getPCIDriver(normalizedAddr)
	if currentDriver == "vfio-pci" {
		return nil // Already bound to vfio-pci
	}

	// Store original driver for later restoration
	if currentDriver != "" {
		originalDriverPath := fmt.Sprintf("/var/run/lab/pci-original-driver/%s", normalizedAddr)
		if err := os.MkdirAll(filepath.Dir(originalDriverPath), 0755); err != nil {
			return fmt.Errorf("failed to create driver storage directory: %w", err)
		}
		if err := os.WriteFile(originalDriverPath, []byte(currentDriver), 0644); err != nil {
			return fmt.Errorf("failed to store original driver: %w", err)
		}

		// Unbind from current driver
		unbindPath := fmt.Sprintf("%s/driver/unbind", devicePath)
		if err := os.WriteFile(unbindPath, []byte(normalizedAddr), 0200); err != nil {
			return fmt.Errorf("failed to unbind from %s: %w", currentDriver, err)
		}
	}

	// Get vendor:device ID for binding
	vendorID, err := os.ReadFile(fmt.Sprintf("%s/vendor", devicePath))
	if err != nil {
		return fmt.Errorf("failed to read vendor ID: %w", err)
	}
	deviceID, err := os.ReadFile(fmt.Sprintf("%s/device", devicePath))
	if err != nil {
		return fmt.Errorf("failed to read device ID: %w", err)
	}

	// Format: vendor device (e.g., "10de 1b80")
	vendorStr := strings.TrimSpace(string(vendorID))
	deviceStr := strings.TrimSpace(string(deviceID))
	newID := fmt.Sprintf("%s %s", vendorStr, deviceStr)

	// Bind to vfio-pci
	newIDPath := "/sys/bus/pci/drivers/vfio-pci/new_id"
	if err := os.WriteFile(newIDPath, []byte(newID), 0200); err != nil {
		// If new_id fails, try bind directly
		bindPath := "/sys/bus/pci/drivers/vfio-pci/bind"
		if bindErr := os.WriteFile(bindPath, []byte(normalizedAddr), 0200); bindErr != nil {
			return fmt.Errorf("failed to bind to vfio-pci: %w (new_id error: %v)", bindErr, err)
		}
	}
...
// UnbindDeviceFromVFIO unbinds a PCI device from vfio-pci and restores the original driver
func (r *PCIRepository) UnbindDeviceFromVFIO(_ context.Context, pciAddr string) error {
	normalizedAddr := normalizePCIAddress(pciAddr)
	if normalizedAddr == "" {
		return fmt.Errorf("invalid PCI address: %s", pciAddr)
	}
	...
	currentDriver := getPCIDriver(normalizedAddr)
	if currentDriver != "vfio-pci" {
		return nil // Not bound to vfio-pci, nothing to do
	}
	...
	originalDriverPath := fmt.Sprintf("/var/run/lab/pci-original-driver/%s", normalizedAddr)
	originalDriver, err := os.ReadFile(originalDriverPath)
	if err != nil {
		// No original driver stored, try common drivers or leave unbound
		// Try to bind to a generic driver
		return nil
	}

	originalDriverStr := strings.TrimSpace(string(originalDriver))

	// Bind to original driver
	bindPath := fmt.Sprintf("/sys/bus/pci/drivers/%s/bind", originalDriverStr)
	if err := os.WriteFile(bindPath, []byte(normalizedAddr), 0200); err != nil {
		// Binding failed, but device is unbound from vfio-pci
		// Clean up the stored driver info
		_ = os.Remove(originalDriverPath)
		return fmt.Errorf("device unbound from vfio-pci but failed to restore to %s: %w", originalDriverStr, err)
	}

	// Clean up stored driver info
	_ = os.Remove(originalDriverPath)
