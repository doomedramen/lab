...
// ListByTargetIP returns all proxy hosts that target a specific IP address.
// The IP is matched against the host portion of target_url (e.g., "http://192.168.1.100:8080").
func (r *ProxyRepository) ListByTargetIP(ctx context.Context, ip string) ([]*model.ProxyHost, error) {
    query := `
        SELECT id, domain, target_url, ssl_mode,
               basic_auth_enabled, basic_auth_user, basic_auth_password_hash,
               custom_request_headers, custom_response_headers,
        ...
        ORDER BY domain
    `
    rows, err := r.db.QueryContext(ctx, query, "%http://"+ip+"%")
    if err != nil {
        return nil, fmt.Errorf("failed to list proxy hosts by target IP: %w", err)
    }
    defer rows.Close()
