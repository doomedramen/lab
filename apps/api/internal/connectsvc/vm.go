...
func (s *VmServiceServer) BindPCIDeviceToVFIO(
	ctx context.Context,
	req *connect.Request[labv1.BindPCIDeviceToVFIORequest],
) (*connect.Response[labv1.BindPCIDeviceToVFIOResponse], error) {
	if req.Msg.Address == "" {
		return nil, connect.NewError(connect.CodeInvalidArgument, errors.New("PCI address is required"))
	}

	if err := s.svc.BindPCIDeviceToVFIO(ctx, req.Msg.Address); err != nil {
		return nil, serviceErrToConnect(err)
	}

	// Get updated device info
	devices, _, _, err := s.svc.ListPCIDevices(ctx)
	if err != nil {
		return connect.NewResponse(&labv1.BindPCIDeviceToVFIOResponse{
			Success: true,
			Message: "Device bound to vfio-pci successfully",
		}), nil
	}

	var updatedDevice *labv1.PCIDevice
	for _, dev := range devices {
		if dev.Address == req.Msg.Address {
			updatedDevice = &labv1.PCIDevice{
				Address:     dev.Address,
				VendorId:    dev.VendorID,
				VendorName:  dev.VendorName,
				ProductId:   dev.ProductID,
...
// UnbindPCIDeviceFromVFIO unbinds a PCI device from vfio-pci
func (s *VmServiceServer) UnbindPCIDeviceFromVFIO(
	ctx context.Context,
	req *connect.Request[labv1.UnbindPCIDeviceFromVFIORequest],
) (*connect.Response[labv1.UnbindPCIDeviceFromVFIOResponse], error) {
	if req.Msg.Address == "" {
		return nil, connect.NewError(connect.CodeInvalidArgument, errors.New("PCI address is required"))
	}

	if err := s.svc.UnbindPCIDeviceFromVFIO(ctx, req.Msg.Address); err != nil {
		return nil, serviceErrToConnect(err)
	}
	...
	devices, _, _, err := s.svc.ListPCIDevices(ctx)
	if err != nil {
		return connect.NewResponse(&labv1.UnbindPCIDeviceFromVFIOResponse{
			Success: true,
			Message: "Device unbound from vfio-pci successfully",
		}), nil
	}
