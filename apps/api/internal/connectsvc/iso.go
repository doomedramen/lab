...
func (s *IsoServiceServer) DownloadISO(
	...
	// Run download in the background. We use context.Background() so that
	// the download continues even after the client disconnects.
	// TODO: inject a server-lifetime context so downloads are cancelled on shutdown.
	go func() {
		_, err := s.svc.DownloadISO(context.Background(), req.Msg.Url, filename)
		if err != nil {
