...
type Repositories struct {
	...
	GitOpsRepo     *sqliteRepo.GitOpsRepository
...
func NewRepositories(ctx context.Context, cfg *config.Config, db *sqlitePkg.DB) (*Repositories, error) {
	...
	if db != nil {
		...
		repos.GitOpsRepo = sqliteRepo.NewGitOpsRepository(db)
	}
	...
	lxcContainerDir := cfg.GetContainersRootDir()
	if lxcClient, err := libvirtx.NewClient(&libvirtx.Config{URI: "lxc:///system"}); err != nil {
		...
		repos.ContainerRepo = libvirt.NewContainerRepository(lxcClient, lxcContainerDir, cfg.GetContainersEmulatorPath())
	}
