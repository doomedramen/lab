...
func main() {
	...
	// Create a shutdown context for background operations
	...
	authInterceptor := NewAuthInterceptor(cfg, repos, jwtSecret)
	...
	// Cancel shutdown context to stop background operations
	cancelShutdown()
...
// validateJWTSecret validates the JWT secret configuration using the validator package
func validateJWTSecret(cfg *config.Config) []byte {
	errs := validator.ValidateJWTSecret(cfg.Auth.JWTSecret)
	if len(errs) > 0 {
		// Format all validation errors for display
		for _, e := range errs {
			log.Printf("JWT secret validation failed: %s", e.Message)
		}
		log.Fatalf("JWT_SECRET validation failed. Generate a secure random secret with: openssl rand -base64 32")
	}
