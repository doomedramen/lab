...
func NewServices(ctx context.Context, deps *ServiceDependencies) (*Services, error) {
	...
	svcs.NodeSvc = service.NewNodeService(repos.NodeRepo, repos.PCIRepo)
	...
	if repos.GitOpsRepo != nil {
		svcs.GitOpsSvc = service.NewGitOpsService(repos.GitOpsRepo)
		// Register reconcilers
		svcs.GitOpsSvc.WithReconciler(service.NewVMReconciler())
		svcs.GitOpsSvc.WithReconciler(service.NewContainerReconciler(svcs.ContainerSvc))
		svcs.GitOpsSvc.WithReconciler(service.NewNetworkReconciler(svcs.NetworkSvc))
		svcs.GitOpsSvc.WithReconciler(service.NewStoragePoolReconciler(svcs.StorageSvc))
		svcs.GitOpsSvc.WithReconciler(service.NewStackReconciler(svcs.StackSvc))
		// Start reconciliation loop in background
		go func() {
			if err := svcs.GitOpsSvc.StartReconciliationLoop(ctx); err != nil {
				log.Printf("GitOps reconciliation loop error: %v", err)
			}
		}()
		log.Println("GitOps service started with StoragePool and DockerStack reconcilers")
	}
