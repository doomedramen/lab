...
import { BiosType, MachineType, NetworkModel, NetworkType, OsType } from "../gen/lab/v1/common_pb"
...
export function containerStatusToString(status: ContainerStatus): string {
  switch (status) {
    ...
    case ContainerStatus.FROZEN: return "frozen"
    default: return "stopped"
  }
}
...
export function stackStatusToString(status: StackStatus): string {
  switch (status) {
    case StackStatus.RUNNING: return "running"
    case StackStatus.PARTIALLY_RUNNING: return "partially_running"
    case StackStatus.STOPPED: return "stopped"
    default: return "stopped"
  }
}
...
export function biosTypeToString(type: BiosType): string {
  switch (type) {
    case BiosType.SEABIOS: return "seabios"
    case BiosType.OVMF: return "ovmf"
    default: return "seabios"
  }
}
...
export function networkModelToString(model: NetworkModel): string {
  switch (model) {
    case NetworkModel.VIRTIO: return "virtio"
    case NetworkModel.E1000: return "e1000"
    case NetworkModel.RTL8139: return "rtl8139"
...
export function osTypeFromString(s: string): OsType {
  switch (s) {
    ...
    case "windows": return OsType.WINDOWS
    case "solaris": return OsType.SOLARIS
    default: return OsType.OTHER
  }
}
...
export function networkModelFromString(s: string): NetworkModel {
  switch (s) {
    case "e1000": return NetworkModel.E1000
    case "rtl8139": return NetworkModel.RTL8139
    default: return NetworkModel.VIRTIO
  }
}
...
export function diskBusToString(bus: DiskBus): string {
  switch (bus) {
    ...
    case DiskBus.IDE: return "ide"
    case DiskBus.USB: return "usb"
    case DiskBus.NVME: return "nvme"
    default: return "virtio"
  }
}
...
export function diskBusFromString(s: string): DiskBus {
  switch (s) {
    case "sata": return DiskBus.SATA
    case "scsi": return DiskBus.SCSI
    case "ide": return DiskBus.IDE
    case "usb": return DiskBus.USB
    case "nvme": return DiskBus.NVME
    default: return DiskBus.VIRTIO
  }
}
...
export function diskFormatToString(format: DiskFormat): string {
  switch (format) {
    ...
    case DiskFormat.VHDX: return "vhdx"
...
export function diskFormatFromString(s: string): DiskFormat {
  switch (s) {
    case "raw": return DiskFormat.RAW
    case "vmdk": return DiskFormat.VMDK
    case "vdi": return DiskFormat.VDI
    case "vhdx": return DiskFormat.VHDX
    default: return DiskFormat.QCOW2
  }
}
