...
export function useNetworks(type?: number, status?: number) {
  return useQuery({
    ...
    queryFn: () => networkClient.listNetworks({ 
      type: type ?? 0, 
      status: status ?? 0 
    }),
    select: (res) => ({
...
export function useNetworkInterfaces(networkId?: string, entityId?: number, entityType?: string) {
  return useQuery({
    queryKey: ["network-interfaces", { networkId, entityId, entityType }],
    queryFn: () => networkClient.listVmNetworkInterfaces({ 
      networkId: networkId ?? "",
      entityId: entityId ?? 0,
...
export function useFirewallRules(scopeType?: string, scopeId?: string, enabledOnly?: boolean) {
  return useQuery({
    queryKey: ["firewall-rules", { scopeType, scopeId, enabledOnly }],
    queryFn: () => firewallClient.listFirewallRules({ 
      scopeType: scopeType ?? "",
      scopeId: scopeId ?? "",
...
export function useFirewallStatus(scopeType?: string, scopeId?: string) {
  return useQuery({
    ...
    queryFn: () => firewallClient.getFirewallStatus({ 
      scopeType: scopeType ?? "",
      scopeId: scopeId ?? "",
    }),
    select: (res) => res,
