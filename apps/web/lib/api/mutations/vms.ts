...
import type { CreateVMResponse, GetVMConsoleResponse, UpdateVMRequest, CloneVMResponse } from "../../gen/lab/v1/vm_pb"
...
export function useVMMutations({ onCreateSuccess, onDeleteSuccess, onUpdateSuccess, onCloneSuccess }: UseVMMutationsOptions = {}) {
  const queryClient = useQueryClient()

  const createVM = useMutation({
    mutationFn: (data: Parameters<typeof vmClient.createVM>[0]) => vmClient.createVM(data),
    onSuccess: (res) => {
  ...
  const getConsole = useMutation({
    mutationFn: ({ vmid, consoleType }: { vmid: number; consoleType?: "serial" | "vnc" | "websockify" }): Promise<GetVMConsoleResponse> => {
