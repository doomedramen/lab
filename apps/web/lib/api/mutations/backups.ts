...
export function useBackupMutations({
  ...
  onDeleteSuccess,
  onRestoreSuccess,
}: UseBackupMutationsOptions = {}) {
  const queryClient = useQueryClient();

  const createBackup = useMutation({
    mutationFn: (data: Parameters<typeof backupClient.createBackup>[0]) =>
      backupClient.createBackup(data),
    onSuccess: (res) => {
  ...
  const restoreBackup = useMutation({
    mutationFn: (data: Parameters<typeof backupClient.restoreBackup>[0]) =>
      backupClient.restoreBackup(data),
    onSuccess: () => {
...
export function useBackupScheduleMutations({
  onCreateSuccess,
  onDeleteSuccess,
  onUpdateSuccess,
}: UseBackupScheduleMutationsOptions = {}) {
  const queryClient = useQueryClient();

  const createSchedule = useMutation({
    mutationFn: (
      data: Parameters<typeof backupClient.createBackupSchedule>[0],
    ) => backupClient.createBackupSchedule(data),
    onSuccess: () => {
  ...
  const updateSchedule = useMutation({
    mutationFn: (
      data: Parameters<typeof backupClient.updateBackupSchedule>[0],
    ) => backupClient.updateBackupSchedule(data),
    onSuccess: () => {
  ...
  const deleteSchedule = useMutation({
    mutationFn: (scheduleId: string) =>
      backupClient.deleteBackupSchedule({ id: scheduleId }),
    onSuccess: () => {
  ...
  const runSchedule = useMutation({
    mutationFn: (scheduleId: string) =>
      backupClient.runBackupSchedule({ id: scheduleId }),
    onSuccess: () => {
