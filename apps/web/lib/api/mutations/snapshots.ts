...
export function useSnapshotMutations({ onCreateSuccess, onDeleteSuccess, onRestoreSuccess }: UseSnapshotMutationsOptions = {}) {
  const queryClient = useQueryClient()

  const createSnapshot = useMutation({
    mutationFn: (data: Parameters<typeof snapshotClient.createSnapshot>[0]) => snapshotClient.createSnapshot(data),
    onSuccess: (res) => {
  ...
  const deleteSnapshot = useMutation({
    mutationFn: ({ vmid, snapshotId, includeChildren }: { vmid: number; snapshotId: string; includeChildren?: boolean }) =>
  ...
  const restoreSnapshot = useMutation({
    mutationFn: ({ vmid, snapshotId, startAfter }: { vmid: number; snapshotId: string; startAfter?: boolean }) =>
