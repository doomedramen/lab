...
export function RestoreBackupModal({ backup, onClose }: RestoreBackupModalProps) {
      ...
      <DialogContent className="sm:max-w-[500px]">
        ...
        <div className="py-4">
          <Alert variant="destructive" className="mb-4">
            ...
            <AlertDescription>
              Restoring to the original VM will overwrite its current state.
          ...
          <div className="bg-muted p-4 rounded-lg space-y-2 mb-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="font-medium">{formatSize(Number(backup.sizeBytes))}</p>
              </div>
              <div>
                <Label className="text-xs text-muted-foreground">Type</Label>
                <p className="font-medium capitalize">{backup.type === 1 ? "Full" : backup.type === 2 ? "Incremental" : "Snapshot"}</p>
              </div>
            </div>
          </div>
