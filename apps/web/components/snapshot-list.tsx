...
import { Clock, MoreVertical, RotateCcw, Trash2, HardDrive, ChevronRight, ChevronDown } from "lucide-react"
...
export function SnapshotList({ vmid, vmName, vmState, snapshots, tree, isLoading }: SnapshotListProps) {
  ...
  const renderSnapshotTree = (treeNode: SnapshotTree | null | undefined, level: number = 0) => {
    ...
    return (
      <div key={snapshot.id}>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-4 mt-1 text-xs text-muted-foreground">
              ...
              {snapshot.description && (
                <span className="truncate max-w-md">{snapshot.description}</span>
              )}
            </div>
          </div>
        ...
        {isExpanded && hasChildren && (
          <div className="mt-2">
            {treeNode.children?.map((child) => renderSnapshotTree(child, level + 1))}
          </div>
        )}
      </div>
    )
