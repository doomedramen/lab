...
import {
  osTypeFromString,
  networkTypeFromString,
  networkModelFromString,
...
export function CreateVMModal({ nodes, trigger }: CreateVMModalProps) {
  ...
  const [networkModel, setNetworkModel] = useState<
  ...
  useEffect(() => {
    if (selectedNode?.arch) {
      const detectedArch =
        selectedNode.arch === "aarch64" ? "aarch64" : "x86_64";
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        setIsUrlValid(false);
        setUrlError(
          "Unable to reach this URL. Please check if it's accessible.",
        );
      } finally {
      ...
      >
        ...
        <Tabs defaultValue="template" className="mt-4">
          <TabsList className="grid grid-cols-4">
            ...
            <TabsTrigger value="resources" disabled={!selectedTemplate}>
              Resources
            </TabsTrigger>
          ...
          <TabsContent value="template" className="space-y-4 mt-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                You must select a node first. Available templates will be
                filtered by the node's architecture.
              </p>
              <Select
                value={node}
              ...
              >
                <SelectTrigger
                  id="template-node"
                  data-testid="vm-template-node-select"
                  className="w-full"
                >
                    ...
                    <SelectItem key={n.id} value={n.name}>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-muted-foreground">
                          ({n.arch || "x86_64"}, {n.cpu?.cores ?? 0} cores,{" "}
                          {(n.memory?.total ?? 0).toFixed(2)} GB RAM)
                        </span>
                      </div>
                    </SelectItem>
                  ))}
              ...
              {!node && (
                <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/20">
                  <p className="text-sm text-amber-600 font-medium">
                    ⚠️ Node selection required
                  </p>
                  <p className="text-xs text-amber-600/80">
                    Select a node above to see compatible VM templates
                  </p>
                </div>
              )}
            </div>
            ...
            {node && (
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Templates provide pre-configured VMs with optimal settings for
                  your node's architecture ({hostArch}).
                </p>
                <Select
                ...
                >
                  <SelectTrigger
                    id="template-select"
                    data-testid="vm-template-select"
                    className="w-full"
                  >
                ...
                <CardContent className="p-4 space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2 pt-2">
                    ...
                    <Badge variant="secondary" className="gap-1">
                      ...
                      {selectedTemplateData.arch === "x86_64"
                        ? "Intel/AMD"
                        : "ARM"}
                    </Badge>
                  </div>
                  {selectedTemplateData.isoUrl ? (
                    <div className="flex items-start gap-2 text-xs text-success">
                      ...
                      <div>
                        <div>
                          ISO will be downloaded for{" "}
                          {hostArch === "aarch64" ? "ARM" : "Intel/AMD"}:
                        </div>
          ...
          <TabsContent value="basic" className="space-y-4 mt-4">
            ...
            {isoSource === "uploaded" ? (
              <div className="space-y-2">
                      ...
                      isos.map((iso) => (
                        <SelectItem key={iso.id} value={iso.id}>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-muted-foreground">
                              (
                              {(Number(iso.size) / 1024 / 1024 / 1024).toFixed(
                                2,
                              )}{" "}
                              GB)
                            </span>
                ...
                {isos && isos.length === 0 && (
                  <p className="text-xs text-amber-600">
                    No ISO images available. Please upload an ISO first or use a
                    template with auto-download.
                  </p>
                )}
              </div>
            ) : (
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  The ISO will be downloaded to the server and stored for future
                  use.
                </p>
              </div>
            )}
          ...
          <TabsContent value="resources" className="space-y-4 mt-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-3 p-3 rounded-lg border border-border bg-secondary/50">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-muted-foreground">
                    {hostArch
                      ? `Detected from host: ${node || "select a node"}`
                      : "Select a node to detect architecture"}
                  </div>
                </div>
              ...
              <p className="text-xs text-muted-foreground">
                Architecture is automatically detected from the selected node.
                VMs run best with native architecture.
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              {cpuError && (
                <p className="text-xs text-destructive">{cpuError}</p>
              )}
              {selectedNode && (
                <p className="text-xs text-muted-foreground">
                  Available: {selectedNode.cpu?.cores ?? 0} cores on{" "}
                  {selectedNode.name}
                </p>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              {selectedNode && (
                <p className="text-xs text-muted-foreground">
                  Available: {(selectedNode.memory?.total ?? 0).toFixed(2)} GB
                  on {selectedNode.name}
                </p>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              {diskError && (
                <p className="text-xs text-destructive">{diskError}</p>
              )}
            </div>
          ...
          <TabsContent value="advanced" className="space-y-4 mt-4">
            ...
            <div className="space-y-3">
              ...
              {pciData && (
                <div className="flex gap-2">
                  <Badge
                    variant={pciData.iommuAvailable ? "default" : "destructive"}
                    className="text-xs"
                  >
                    IOMMU:{" "}
                    {pciData.iommuAvailable ? "Available" : "Not Available"}
                  </Badge>
                  <Badge
                    variant={pciData.vfioAvailable ? "default" : "secondary"}
                    className="text-xs"
                  >
              ...
              {!pciData?.iommuAvailable && (
                <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/20 text-amber-700 dark:text-amber-400 text-xs">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      IOMMU not detected. Enable IOMMU in BIOS and add{" "}
                    ...
                    <div key={group} className="p-2">
                      ...
                      {devices.map((device) => (
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={selectedPCIDevices.includes(
                              device.address,
                            )}
                            onChange={(e) => {
                              if (e.target.checked) {
              ...
              {selectedPCIDevices.length > 0 && (
                <div className="flex flex-wrap gap-1">
                  {selectedPCIDevices.map((addr) => {
                    const device = pciData?.devices?.find(
                      (d) => d.address === addr,
                    );
                    return (
                      <Badge key={addr} variant="secondary" className="text-xs">
                        ...
                        <button
                          ...
                          onClick={() =>
                            setSelectedPCIDevices(
                              selectedPCIDevices.filter((a) => a !== addr),
                            )
                          }
                        >
                          ×
                ...
                {bootOrder.map((device, index) => (
                  <Badge key={device} variant="secondary" className="gap-1.5">
                    ...
                    <button
                      ...
                      className="ml-1 hover:text-destructive"
                      onClick={() =>
                        setBootOrder(bootOrder.filter((d) => d !== device))
                      }
                    >
                      ×
                    </button>
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <Select
                  value={networkType}
                  onValueChange={(v) => setNetworkType(v as "user" | "bridge")}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                ...
                <Select
                  value={networkModel}
                  onValueChange={(v) =>
                    setNetworkModel(v as "virtio" | "e1000" | "rtl8139")
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
        ...
        <DialogFooter className="mt-4">
          ...
          <Button
            ...
            disabled={
              ...
              (isoSource === "url" &&
                (!isoUrl.trim() || isUrlValid === false || isUrlChecking))
            }
          >
            {isCreating ? (
              <>
