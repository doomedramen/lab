  ...
  return (
    <div className="flex flex-col gap-0.5 py-1">
      {nodes.map((node) => {
        ...
        const allNodeCts =
          containers?.filter((c) => c.node === node.name) || [];

        // Filter by search query
        const nodeVms = q
          ? allNodeVms.filter(
              (v) =>
                v.name.toLowerCase().includes(q) || String(v.vmid).includes(q),
            )
          : allNodeVms;
        const nodeCts = q
          ? allNodeCts.filter(
              (c) =>
                c.name.toLowerCase().includes(q) || String(c.ctid).includes(q),
            )
          : allNodeCts;
        ...
        const isExpanded = q
          ? nodeNameMatches || hasMatchingChildren
