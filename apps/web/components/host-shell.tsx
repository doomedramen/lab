...
export function HostShell({ nodeId, nodeName }: HostShellProps) {
  ...
  useEffect(() => {
    ...
    getToken.mutate(nodeId, {
      ...
      onError: (err) => {
        setTokenError(
          err instanceof Error ? err.message : "Failed to get shell token",
        );
      },
    });
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-xs text-muted-foreground">
        Use this terminal to run commands directly on the host system. Type{" "}
        <code className="px-1 py-0.5 bg-secondary rounded">exit</code> to close
        the session.
      </p>
    </div>
  );
