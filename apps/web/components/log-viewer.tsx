...
function getLogLevelColor(level: VMLogLevel): string {
  return (
    ...
    LOG_LEVEL_COLORS[VMLogLevel.VM_LOG_LEVEL_UNSPECIFIED]
  );
}
    ...
    <div className="border-b border-border/50">
      ...
      >
        ...
        <div className="shrink-0">
          <Badge
            variant="outline"
            className={`${levelColor} text-white border-0 text-[10px] px-1.5 py-0 h-4`}
          >
        ...
        <div className="shrink-0">
          <Badge
            variant="outline"
            className={`${sourceColor} text-white border-0 text-[10px] px-1.5 py-0 h-4`}
          >
            {entry.source}
          </Badge>
        </div>

        <div className="flex-1 truncate text-foreground">{entry.message}</div>
      </div>
...
export function LogViewer({ entries, isLoading, onRefresh }: LogViewerProps) {
  ...
  const filteredEntries = useMemo(() => {
    ...
    return sorted.filter((entry) => {
      ...
      if (
        ...
        !selectedSources.includes(entry.source)
      ) {
        return false;
      }
  ...
  const hasActiveFilters =
    searchQuery || selectedLevels.length > 0 || selectedSources.length > 0;
        ...
        <Select
          ...
          onValueChange={(val) => {
            if (val === "all") setSelectedLevels([]);
              ...
              setSelectedLevels([
                VMLogLevel.VM_LOG_LEVEL_ERROR,
                VMLogLevel.VM_LOG_LEVEL_CRITICAL,
              ]);
            else if (val === "warnings")
              setSelectedLevels([
                VMLogLevel.VM_LOG_LEVEL_WARNING,
                VMLogLevel.VM_LOG_LEVEL_ERROR,
                VMLogLevel.VM_LOG_LEVEL_CRITICAL,
              ]);
          }}
        ...
        <div className="flex items-center gap-2 p-2 border-b border-border shrink-0 flex-wrap">
          ...
          {uniqueSources.map((source) => (
            <div key={source} className="flex items-center gap-1">
              ...
              >
                <div
                  className={`size-2 rounded-full ${getSourceColor(source)}`}
                />
                {source}
              </label>
            </div>
