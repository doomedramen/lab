...
export function CreateStackModal({ open, onOpenChange, onCreated }: CreateStackModalProps) {
  ...
  const validateName = (value: string) => {
    if (!/^[a-zA-Z0-9_-]+$/.test(value)) {
      setNameError("Only letters, numbers, hyphens, and underscores are allowed")
      return false
    }
    setNameError("")
    return true
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] flex flex-col" data-testid="create-stack-modal">
        <DialogHeader>
          <DialogTitle>Create Stack</DialogTitle>
          <DialogDescription>
            Create a new Docker Compose stack. The stack folder and files will be created in the configured stacks directory.
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleCreate} disabled={isCreating || !name.trim()} data-testid="stack-create-submit">
            {isCreating ? (
              <>
