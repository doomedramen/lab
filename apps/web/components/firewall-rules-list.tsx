...
export function FirewallRulesList({
  rules,
  isLoading,
}: FirewallRulesListProps) {
  const { deleteRule, enableRule, disableRule, isDeleting } =
    useFirewallMutations();
  ...
  const getDirectionBadge = (direction: number) => {
    ...
    return (
      <Badge variant="outline">{directionMap[direction] ?? "Unknown"}</Badge>
    );
  };
