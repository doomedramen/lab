...
export function ISOUploadModal({ open, onOpenChange, onSuccess }: ISOUploadModalProps) {
    ...
    if (open && !uppyRef.current) {
      ...
      uppyRef.current.on("upload-progress", (file, progress) => {
        if (file && progress.bytesTotal) {
          ...
          setUploads((prev) =>
            prev.map((u) => (u.fileId === file.id ? { ...u, progress: percent } : u))
          )
        }
      })
      ...
      uppyRef.current.on("upload-success", (file) => {
        if (file) {
          setUploads((prev) =>
            prev.map((u) => (u.fileId === file.id ? { ...u, status: "complete", progress: 100 } : u))
          )
          onSuccess?.()
        }
      })
      ...
      uppyRef.current.on("upload-error", (file, error) => {
        if (file) {
          setUploads((prev) =>
            prev.map((u) =>
              u.fileId === file.id ? { ...u, status: "error", error: String(error) } : u
          ))
        }
      })
    }
  ...
  const handleFiles = useCallback((files: FileList | null) => {
    ...
    Array.from(files).forEach((file) => {
      ...
      if (file.name.endsWith(".iso") || file.type === "application/octet-stream") {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Drag and drop ISO files to upload. Files are uploaded using resumable Tus protocol.
          </DialogDescription>
        </DialogHeader>
