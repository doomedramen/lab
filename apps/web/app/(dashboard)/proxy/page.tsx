...
import {
  Globe, Plus, Trash2, Edit, CheckCircle2, XCircle,
  Loader2, Lock, Activity, ArrowUpCircle, ArrowDownCircle,
  PauseCircle, Clock,
} from "lucide-react"
import { PageHeader } from "@/components/page-header"
import { useProxyHosts, useProxyStatus, useMonitors, useMonitorStats, useMonitorHistory } from "@/lib/api/queries/proxy"
...
function sslModeLabel(mode: ProxySSLMode): string {
  switch (mode) {
    case ProxySSLMode.PROXY_SSL_MODE_NONE:        return "HTTP Only"
    case ProxySSLMode.PROXY_SSL_MODE_SELF_SIGNED: return "Self-Signed"
    case ProxySSLMode.PROXY_SSL_MODE_ACME:        return "ACME / Let's Encrypt"
    case ProxySSLMode.PROXY_SSL_MODE_CUSTOM:      return "Custom Cert"
    default:                                      return "Unknown"
  }
}

function sslModeBadgeVariant(mode: ProxySSLMode): "default" | "secondary" | "outline" | "destructive" {
  switch (mode) {
    case ProxySSLMode.PROXY_SSL_MODE_NONE:        return "secondary"
    case ProxySSLMode.PROXY_SSL_MODE_SELF_SIGNED: return "outline"
    case ProxySSLMode.PROXY_SSL_MODE_ACME:        return "default"
    case ProxySSLMode.PROXY_SSL_MODE_CUSTOM:      return "default"
...
interface StatusBadgeProps { id: string }
function StatusBadge({ id }: StatusBadgeProps) {
  ...
  if (status.isRunning && status.backendReachable) {
    return <Badge variant="default" className="gap-1"><CheckCircle2 className="size-3" />Online</Badge>
  }
  if (!status.backendReachable) {
    return <Badge variant="destructive" className="gap-1"><XCircle className="size-3" />Unreachable</Badge>
  }
  return <Badge variant="secondary" className="gap-1"><XCircle className="size-3" />Offline</Badge>
}

interface MonitorStatsRowProps { monitor: UptimeMonitor }
function MonitorStatsRow({ monitor }: MonitorStatsRowProps) {
  const { data: stats, isLoading } = useMonitorStats(monitor.id)
  if (isLoading) return (
    <>
      <TableCell><Loader2 className="size-4 animate-spin text-muted-foreground" /></TableCell>
      <TableCell>—</TableCell>
      <TableCell>—</TableCell>
    </>
  )
  return (
    <>
      ...
      <TableCell className="text-sm text-muted-foreground font-mono">
        {stats?.avgResponseMs24h ? `${Math.round(stats.avgResponseMs24h)}ms` : "—"}
      </TableCell>
    </>
  )
}

interface MonitorHistoryChartProps { monitorId: string }
function MonitorHistoryChart({ monitorId }: MonitorHistoryChartProps) {
  const { data: results } = useMonitorHistory(monitorId, 50)
  if (!results || results.length === 0) {
    return <p className="text-xs text-muted-foreground py-2">No history yet.</p>
  }
...
function ProxyHostsTab() {
  ...
  const { createProxyHost, updateProxyHost, deleteProxyHost, isCreating, isUpdating, isDeleting } =
    useProxyMutations()
  ...
  function handleDelete() {
    ...
    deleteProxyHost.mutate({ id: deleteTarget }, { onSuccess: () => setDeleteTarget(null) })
  }
    ...
    <>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {hosts.filter((h) => h.sslMode !== ProxySSLMode.PROXY_SSL_MODE_NONE && h.sslMode !== ProxySSLMode.PROXY_SSL_MODE_UNSPECIFIED).length}
            </div>
          </CardContent>
        </Card>
      </div>
              ...
              <TableBody>
                {hosts.map((host) => (
                  <TableRow key={host.id}>
                    ...
                    <TableCell className="text-muted-foreground text-sm font-mono">{host.targetUrl}</TableCell>
                    <TableCell>
      ...
      <Dialog open={!!editTarget} onOpenChange={(o) => !o && setEditTarget(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>Update the proxy host configuration.</DialogDescription>
          </DialogHeader>
          <ProxyHostForm form={form} onChange={setForm} />
          <DialogFooter>
      ...
      <Dialog open={!!deleteTarget} onOpenChange={(o) => !o && setDeleteTarget(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This will remove the proxy host and its associated certificate. This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
...
function UptimeMonitorsTab() {
  ...
  const { createMonitor, updateMonitor, deleteMonitor, isCreating, isUpdating, isDeleting } =
    useMonitorMutations()
  ...
  function handleDelete() {
    if (!deleteTarget) return
    deleteMonitor.mutate({ id: deleteTarget }, { onSuccess: () => setDeleteTarget(null) })
  }
                ...
                {monitors.map((m) => (
                  <>
                    <TableRow key={m.id}>
                      ...
                      <TableCell className="text-muted-foreground text-sm font-mono max-w-xs truncate">{m.url}</TableCell>
                      <TableCell className="text-sm text-muted-foreground">
      ...
      <Dialog open={!!deleteTarget} onOpenChange={(o) => !o && setDeleteTarget(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This will remove the monitor and all its history. This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setDeleteTarget(null)}>Cancel</Button>
  ...
  return (
    <div className="grid gap-4 md:grid-cols-3">
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-2xl font-bold">{monitors.filter((m) => m.enabled).length}</div>
        </CardContent>
      </Card>
      <Card>
        <CardHeader className="pb-2">
        ...
        <CardContent>
          <div className="text-2xl font-bold">
            {monitors.length === 0
              ? "—"
              : formatInterval(Math.min(...monitors.filter(m => m.enabled).map(m => m.intervalSeconds)))}
          </div>
        </CardContent>
      </Card>
    </div>
...
function ProxyHostForm({ form, onChange }: ProxyHostFormProps) {
  function set<K extends keyof ProxyFormState>(key: K, value: ProxyFormState[K]) {
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-1.5">
        ...
        >
          ...
          <SelectContent>
            <SelectItem value={String(ProxySSLMode.PROXY_SSL_MODE_NONE)}>HTTP Only</SelectItem>
            <SelectItem value={String(ProxySSLMode.PROXY_SSL_MODE_SELF_SIGNED)}>Self-Signed Certificate</SelectItem>
            <SelectItem value={String(ProxySSLMode.PROXY_SSL_MODE_ACME)}>ACME / Let's Encrypt</SelectItem>
            <SelectItem value={String(ProxySSLMode.PROXY_SSL_MODE_CUSTOM)}>Custom Certificate</SelectItem>
          </SelectContent>
        </Select>
      </div>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">Forward WebSocket upgrade headers</p>
        </div>
        <Switch
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">Protect this host with a username and password</p>
        </div>
        <Switch
      ...
      <div className="flex items-center justify-between">
        <div>
          <Label>Enabled</Label>
          <p className="text-xs text-muted-foreground">Route traffic through this proxy host</p>
        </div>
        <Switch
...
function MonitorForm({ form, onChange }: MonitorFormProps) {
  function set<K extends keyof MonitorFormState>(key: K, value: MonitorFormState[K]) {
    onChange({ ...form, [key]: value })
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <Label>Enabled</Label>
          <p className="text-xs text-muted-foreground">Actively check this URL</p>
        </div>
        <Switch
          checked={form.enabled}
          onCheckedChange={(v) => set("enabled", v)}
