        ...
        <TabsContent value="summary" className="space-y-4 mt-4">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <ResourceMetricCard
              label="CPU Usage"
              value={`${ct.cpu?.used ?? 0}%`}
              subtitle={`${ct.cpu?.cores ?? 0} vCPUs`}
              icon={<Cpu className="size-4" />}
            />
            <ResourceMetricCard
              label="Memory"
              value={`${ct.memory?.used ?? 0} GB`}
              subtitle={`of ${ct.memory?.total ?? 0} GB`}
              icon={<MemoryStick className="size-4" />}
            />
            <ResourceMetricCard
              label="Disk"
              value={`${ct.disk?.used ?? 0} GB`}
              subtitle={`of ${ct.disk?.total ?? 0} GB`}
          ...
          <div className="grid gap-4 lg:grid-cols-2">
              ...
              <CardContent>
                <ConfigList
                  items={[
                    ...
                    {
                      label: "Unprivileged",
                      value: ct.unprivileged ? "Yes" : "No",
                    },
                    {
                      label: "Start on Boot",
                      value: ct.startOnBoot ? "Yes" : "No",
                    },
                    {
                      label: "Swap",
                      value: `${ct.swap?.used ?? 0} / ${ct.swap?.total ?? 0} GB`,
                    },
                  ]}
                />
              </CardContent>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <ResourceBar
                  label="Memory"
                  used={ct.memory?.used ?? 0}
                  total={ct.memory?.total ?? 1}
                  unit="GB"
                />
                <ResourceBar
                  label="Disk"
                  used={ct.disk?.used ?? 0}
                  total={ct.disk?.total ?? 1}
                  unit="GB"
                />
                <ResourceBar
                  label="Swap"
                  used={ct.swap?.used ?? 0}
                  total={ct.swap?.total ?? 1}
                  unit="GB"
                />
                <div className="pt-2">
            ...
            <CardContent>
              <div className="space-y-4">
                <ResourceConfigItem
                  ...
                  label="CPU"
                  value={`${ct.cpu?.cores ?? 0} cores`}
                  detail="Unlimited CPU limit"
                />
                <ResourceConfigItem
                  icon={
                  ...
                  label="Memory"
                  value={`${ct.memory?.total ?? 0} GB`}
                  detail={`Swap: ${ct.swap?.total ?? 0} GB`}
                />
                <ResourceConfigItem
                  icon={<HardDrive className="size-4 text-muted-foreground" />}
                  label="Root Disk"
                  value={`${ct.disk?.total ?? 0} GB`}
                  detail="Storage: local-lvm"
                />
                <ResourceConfigItem
                  icon={<Network className="size-4 text-muted-foreground" />}
                  label="Network"
                  value={ct.ip}
                  detail="Bridge: vmbr0, DHCP: No"
                />
              </div>
            </CardContent>
          </Card>
                ...
                <div className="rounded-md border border-border bg-secondary/30 px-4 py-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <StatusBadge
                        status={
                          ct.status === ContainerStatus.RUNNING
                            ? "online"
                            : "offline"
                        }
                      />
...
function ContainerDetailView() {
  ...
  return (
    <Shimmer
      loading={isLoading}
      templateProps={{ ct: templateContainer, mutationProps: {} }}
    >
