...
import { StatusBadge, ResourceUsageBar, TagList } from "@/components/lab-shared"
...
import { OsType, NetworkType, NetworkModel, MachineType, BiosType } from "@/lib/gen/lab/v1/common_pb"
...
const templateVMs = [
  {
    ...
    network: [{ type: NetworkType.USER, model: NetworkModel.VIRTIO, bridge: "", vlan: 0 }],
    ip: "10.0.10.100",
    tags: ["production", "web"],
    ha: true,
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-foreground text-balance" data-testid="vms-page-title">Virtual Machines</h1>
          <p className="text-sm text-muted-foreground mt-1" data-testid="vms-page-summary">
      ...
      <div className="rounded-lg border border-border bg-card overflow-hidden" data-testid="vms-table">
        <Table>
          <TableHeader>
            ...
            {vms.map((vm) => {
              ...
              return (
                <TableRow key={vm.id} data-testid={`vm-table-row-${vm.vmid}`}>
                  ...
                  <TableCell>
                    <Link href={`/vms/${vm.vmid}`} className="flex items-center gap-2 hover:text-primary transition-colors" data-testid={`vm-table-link-${vm.vmid}`}>
                      ...
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground" data-testid={`vm-table-os-${vm.vmid}`}>
                  ...
                  <TableCell>
                    <StatusBadge status={vmStatusToString(vm.status)} data-testid={`vm-table-status-${vm.vmid}`} />
                  </TableCell>
                  <TableCell>
                    {node ? (
                      ...
                      <span className="text-sm text-muted-foreground">{vm.node}</span>
                    )}
                  </TableCell>
                  <TableCell>
                    <div className="space-y-1">
                      <ResourceUsageBar value={vm.cpu?.used ?? 0} />
                      <div className="text-[11px] text-muted-foreground">{vm.cpu?.cores ?? 0} cores</div>
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="space-y-1">
                      <ResourceUsageBar value={Math.round(((vm.memory?.used ?? 0) / (vm.memory?.total || 1)) * 100)} />
                  ...
                  <TableCell>{vm.ha && <Shield className="size-4 text-primary" />}</TableCell>
                  <TableCell>
...
export default function VMsPage() {
  const { data: vms, isLoading: vmsLoading, error: vmsError, refetch: refetchVMs } = useVMs()
  const { data: nodes } = useNodes()

  if (vmsError) {
    return (
      <div className="p-6">
        <ErrorDisplay message={vmsError.message} onRetry={() => refetchVMs()} className="h-[50vh]" />
      </div>
    )
  }

  return (
    <Shimmer loading={vmsLoading} templateProps={{ vms: templateVMs, nodes: nodes || [] }}>
