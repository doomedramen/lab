...
import { StatusBadge, ResourceUsageBar } from "@workspace/components/lab-shared"
...
function HostsContent({ nodes }: { nodes: Node[] }) {
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-foreground text-balance">Host Nodes</h1>
        <p className="text-sm text-muted-foreground mt-1">{nodes.length} nodes in cluster</p>
      </div>
              ...
              <TableRow key={node.id}>
                ...
                <TableCell>
                  <div className="space-y-1">
                    ...
                    <div className="text-[11px] text-muted-foreground">{node.cpu?.cores ?? 0} cores</div>
                  </div>
                </TableCell>
                <TableCell>
                  <div className="space-y-1">
                    <ResourceUsageBar value={Math.round(((node.memory?.used ?? 0) / (node.memory?.total || 1)) * 100)} />
                ...
                <TableCell>
                  <div className="space-y-1">
                    <ResourceUsageBar value={Math.round(((node.disk?.used ?? 0) / (node.disk?.total || 1)) * 100)} />
                    <div className="text-[11px] text-muted-foreground">
