...
import { StatusBadge, ResourceBar, TagList } from "@workspace/components/lab-shared"
...
function HostDetailContent({ node, nodeVms, nodeCts, cpuChartData, memoryChartData, mutationProps }: HostDetailContentProps) {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex justify-end gap-2">
        <Button variant="outline" size="sm" className="gap-1.5" onClick={onReboot} disabled={loading.reboot}>
          {loading.reboot ? <Loader2 className="size-3.5 animate-spin" /> : <RotateCcw className="size-3.5" />}
          Reboot
        </Button>
        <Button variant="outline" size="sm" className="gap-1.5" onClick={onShutdown} disabled={loading.shutdown}>
          {loading.shutdown ? <Loader2 className="size-3.5 animate-spin" /> : <Power className="size-3.5" />}
          Shutdown
        </Button>
      ...
      <Tabs defaultValue="summary" className="flex flex-col">
        <TabsList>
          ...
          <TabsTrigger value="containers">Containers ({nodeCts.length})</TabsTrigger>
        ...
        <TabsContent value="summary" className="space-y-4 mt-4">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <ResourceMetricCard label="CPU Cores" value={node.cpu?.cores ?? 0} subtitle={`${node.cpu?.used ?? 0}% used`} icon={<Cpu className="size-4" />} />
            <ResourceMetricCard label="Memory" value={`${Math.round(node.memory?.used ?? 0)} GB`} subtitle={`of ${node.memory?.total ?? 0} GB`} icon={<MemoryStick className="size-4" />} />
            <ResourceMetricCard label="Disk" value={`${node.disk?.used ?? 0} TB`} subtitle={`of ${node.disk?.total ?? 0} TB`} icon={<HardDrive className="size-4" />} />
            <ResourceMetricCard label="Uptime" value={node.uptime} subtitle={`Kernel ${node.kernel}`} icon={<Clock className="size-4" />} />
          </div>
            ...
            <Card>
              ...
              <CardContent>
                <ConfigList items={[
                  ...
                  { label: "Network In", value: formatNetwork(node.networkIn) },
                  { label: "Network Out", value: formatNetwork(node.networkOut) },
                ]} />
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent className="space-y-4">
                <ResourceBar label="CPU" used={node.cpu?.used ?? 0} total={100} unit="%" showPercent={false} />
                <ResourceBar label="Memory" used={Math.round(node.memory?.used ?? 0)} total={node.memory?.total ?? 1} unit="GB" />
                <ResourceBar label="Disk" used={node.disk?.used ?? 0} total={node.disk?.total ?? 1} unit="TB" />
                <div className="pt-2 flex items-center gap-4">
                  <div className="flex items-center gap-1.5">
                    <Monitor className="size-4 text-muted-foreground" />
                    <span className="text-sm text-foreground">{nodeVms.length} VMs</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <Box className="size-4 text-muted-foreground" />
                    <span className="text-sm text-foreground">{nodeCts.length} Containers</span>
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <TableBody>
                {nodeVms.map((vm) => (
                  <TableRow key={vm.id}>
                    ...
                    <TableCell><StatusBadge status={vmStatusToString(vm.status)} /></TableCell>
                    <TableCell className="text-sm text-foreground">{vm.cpu?.used ?? 0}% ({vm.cpu?.cores ?? 0}c)</TableCell>
                    <TableCell className="text-sm text-foreground">{vm.memory?.used ?? 0}/{vm.memory?.total ?? 0} GB</TableCell>
                    <TableCell><TagList tags={vm.tags} /></TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
              ...
              <TableBody>
                {nodeCts.map((ct) => (
                  <TableRow key={ct.id}>
                    ...
                    <TableCell><StatusBadge status={containerStatusToString(ct.status)} /></TableCell>
                    <TableCell className="text-sm text-foreground">{ct.cpu?.used ?? 0}% ({ct.cpu?.cores ?? 0}c)</TableCell>
                    <TableCell className="text-sm text-foreground">{ct.memory?.used ?? 0}/{ct.memory?.total ?? 0} GB</TableCell>
                    <TableCell><TagList tags={ct.tags} /></TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
...
export default function HostDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { rebootNode, shutdownNode, isRebooting, isShuttingDown } = useNodeMutations()
  ...
  const nodeVms = node && allVms ? allVms.filter((v) => v.node === node.name) : templateVms
  const nodeCts = node && allContainers ? allContainers.filter((c) => c.node === node.name) : templateContainers

  const handleReboot = () => node && rebootNode.mutate(node.id)
  const handleShutdown = () => node && shutdownNode.mutate(node.id)

  return (
    <Shimmer loading={isLoading} templateProps={{ node: templateNode, nodeVms: templateVms, nodeCts: templateContainers, cpuChartData: [], memoryChartData: [], mutationProps: { onReboot: () => {}, onShutdown: () => {}, loading: { reboot: false, shutdown: false } } }}>
      <HostDetailContent
