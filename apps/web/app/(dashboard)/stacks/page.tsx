...
function StacksContent({ stacks }: { stacks: Stack[] }) {
  ...
  const partial = stacks.filter(
    (s) => s.status === StackStatus.PARTIALLY_RUNNING,
  ).length;
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1
            ...
            data-testid="stacks-page-title"
          >
            Stacks
          </h1>
          <p className="text-sm text-muted-foreground mt-1">
            {stacks.length} total — {running} running, {partial} partial,{" "}
            {stopped} stopped
          </p>
        </div>
        <Button
                    ...
                    <div className="space-y-1">
                      ...
                      {stack.containers.length > 4 && (
                        <p className="text-xs text-muted-foreground">
                          +{stack.containers.length - 4} more
                        </p>
                      )}
                    </div>
                  )}
