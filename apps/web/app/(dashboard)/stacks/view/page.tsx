...
function StackDetailContent({ stack }: StackDetailContentProps) {
  ...
  const [bashContainer, setBashContainer] = useState<{
    id: string;
    name: string;
  } | null>(null);
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1
                ...
                data-testid="stack-detail-name"
              >
                {stack.name}
              </h1>
              <StatusBadge
                status={stackStatusToString(stack.status)}
                data-testid="stack-detail-status"
              />
            </div>
            <p
              className="text-xs text-muted-foreground font-mono"
              data-testid="stack-detail-id"
            >
              {stack.id}
            </p>
          </div>
          ...
          {stack.containers.length === 0 ? (
            <div className="rounded-lg border border-border bg-secondary/10 p-8 text-center">
              <p className="text-sm text-muted-foreground">
                No containers. Start the stack to bring up containers.
              </p>
            </div>
          ) : (
        ...
        <TabsContent value="compose" className="mt-4 space-y-3">
          ...
          <div className="flex justify-end">
            <Button
              size="sm"
              onClick={() =>
                updateStack.mutate({ id: stack.id, compose, env: stack.env })
              }
              disabled={isUpdating}
            >
              {isUpdating ? (
      ...
      <AlertDialog open={confirmDown} onOpenChange={setConfirmDown}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will run{" "}
              <code className="font-mono text-sm">docker compose down</code>,
              stopping and removing all containers for{" "}
              <strong>{stack.name}</strong>. The compose file will be preserved.
      ...
      <AlertDialog open={confirmDelete} onOpenChange={setConfirmDelete}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete the <strong>{stack.name}</strong>{" "}
              stack folder, including the compose file and all configuration.
