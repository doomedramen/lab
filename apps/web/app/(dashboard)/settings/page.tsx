...
export default function SettingsPage() {
  return (
    <Suspense fallback={<div className="p-6"><div className="h-48 animate-pulse rounded-lg bg-muted" /></div>}>
      <SettingsContent />
    </Suspense>
  )
}
      ...
      <Card>
        ...
        <CardContent>
          <form onSubmit={handlePasswordChange} className="space-y-4 max-w-sm">
            ...
            <Button
              type="submit"
              disabled={!currentPassword || !newPassword || !confirmPassword || updateProfile.isPending}
            >
              Change password
            </Button>
          </form>
          ...
          <CardContent className="space-y-4">
            <div className="flex flex-col items-start gap-4">
              ...
              <div className="text-sm">
                <p className="text-muted-foreground mb-1">Can&apos;t scan? Enter this key manually:</p>
                <code className="bg-muted px-2 py-1 rounded text-xs font-mono">{setupData.manualKey}</code>
              </div>
            </div>
            ...
            <form onSubmit={handleEnable} className="space-y-3">
              ...
              <div className="flex gap-2">
                <Button type="submit" disabled={verifyCode.length !== 6 || enableMFA.isPending}>
                  Verify and enable
                </Button>
                <Button
                  type="button"
      ...
      {createdRawKey && (
        <Card className="border-success/30 bg-success/5">
          ...
          <CardContent className="space-y-3">
            ...
            <Button size="sm" variant="ghost" onClick={() => setCreatedRawKey(null)}>
              I&apos;ve saved it
            </Button>
          </CardContent>
        </Card>
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          This will immediately revoke &quot;{key.name}&quot;. Any scripts or applications using it will stop working.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
          ...
          {currentSession ? (
            <div className="flex items-start gap-3 py-2">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <p className="text-sm font-medium">
                    {currentSession.deviceName || `${getBrowserInfo(currentSession.userAgent)} on ${getOSInfo(currentSession.userAgent)}`}
                  </p>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  ...
                  {currentSession.ipAddress && currentSession.userAgent && " · "}
                  {getBrowserInfo(currentSession.userAgent)} · {getOSInfo(currentSession.userAgent)}
                </p>
                <p className="text-xs text-muted-foreground">
            ...
            <div className="divide-y">
              {otherSessions.map((session: Session) => (
                <div key={session.id} className="flex items-start gap-3 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium">
                      {session.deviceName || `${getBrowserInfo(session.userAgent)} on ${getOSInfo(session.userAgent)}`}
                    </p>
                    <p className="text-xs text-muted-foreground mt-1">
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {sessionToRevoke && (
                <>
                  This will sign out the device at {sessionToRevoke.ipAddress || "unknown IP"}.
      ...
      <AlertDialog open={showRevokeAllDialog} onOpenChange={setShowRevokeAllDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will sign out {otherSessionCount} other device{otherSessionCount !== 1 ? "s" : ""}.
