...
function TaskStatusBadge({ status }: { status: TaskStatus }) {
  ...
  const { label, variant, icon } =
    config[status] || config[TaskStatus.UNSPECIFIED];
...
function ResourceTypeBadge({ resourceType }: { resourceType: number }) {
  ...
  return (
    <Badge variant="outline">{typeLabels[resourceType] || "Unknown"}</Badge>
  );
}
      ...
      {tasks.map((task) => (
        <TableRow key={task.id}>
          ...
          <TableCell className="text-sm text-muted-foreground">
            {task.createdAt &&
              formatDistanceToNow(timestampToDate(task.createdAt), {
                addSuffix: true,
              })}
          </TableCell>
          <TableCell className="text-sm text-muted-foreground">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Monitor and manage async operations
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Button
      ...
      <div className="rounded-md border">
        <Table>
          ...
          <TableBody>
            <Shimmer
              loading={isLoading}
              templateProps={{ tasks: templateTasks }}
            >
              <TasksTableBody
