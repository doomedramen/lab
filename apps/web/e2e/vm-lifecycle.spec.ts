...
test.describe("VM Lifecycle — create, start, stop, delete", () => {
  ...
  test("full lifecycle: create → start → stop → delete", async ({ page }) => {
    ...
    await expect(page.getByTestId("confirm-dialog-title")).toContainText(/force stop/i)
    ...
    await expect(page.getByTestId("confirm-dialog")).toBeVisible()
    await expect(page.getByTestId("confirm-dialog-title")).toContainText(/delete/i)
    ...
    console.log(`[lifecycle] VM vmid=${createdVmid} deleted, redirected to /vms`)
  ...
  test("delete button states during VM lifecycle", async ({ createAndStartVM, page }) => {
    ...
    await expect(page.getByTestId("vm-delete-button")).toBeDisabled()
    await expect(page.getByTestId("vm-delete-button")).toHaveAttribute("data-disabled", "true")
    ...
    await expect(page.getByTestId("vm-delete-button")).toBeEnabled()
    await expect(page.getByTestId("vm-delete-button")).not.toHaveAttribute("data-disabled", "true")
  })

  test("delete confirmation dialog validation", async ({ createAndStartVM, page }) => {
    createdVmid = await createAndStartVM()
    ...
    await expect(page.getByTestId("confirm-dialog")).toBeVisible()
    await expect(page.getByTestId("confirm-dialog-title")).toContainText(/delete/i, { timeout: 5000 })
  ...
  test("confirmation dialog cancel leaves VM unchanged", async ({ createAndStartVM, page }) => {
    createdVmid = await createAndStartVM()
...
test.describe("VM Lifecycle — pause and resume", () => {
  ...
  test("can pause a running VM and resume it", async ({ createAndStartVM, page }) => {
    createdVmid = await createAndStartVM()
...
test.describe("VM Lifecycle — graceful shutdown", () => {
  ...
  test("can gracefully shutdown a running VM", async ({ createAndStartVM, page }) => {
    createdVmid = await createAndStartVM()
    console.log(`[graceful-shutdown] vmid=${createdVmid} running, sending shutdown`)
    ...
    // Confirmation dialog
    await expect(page.getByTestId("confirm-dialog")).toBeVisible()
    await expect(page.getByTestId("confirm-dialog-title")).toContainText(/shutdown/i)
    await page.getByTestId("confirm-dialog-confirm").click()
...
test.describe("VM Lifecycle — reboot", () => {
  ...
  test("can reboot a running VM", async ({ createAndStartVM, page }) => {
    ...
    // Confirmation dialog
    await expect(page.getByTestId("confirm-dialog")).toBeVisible()
    await expect(page.getByTestId("confirm-dialog-title")).toContainText(/reboot/i)
    await page.getByTestId("confirm-dialog-confirm").click()
...
test.describe("VM button states", () => {
  ...
  test("delete button is disabled when VM is running", async ({ createAndStartVM, page }) => {
    createdVmid = await createAndStartVM()
    await expect(page.getByTestId("vm-delete-button")).toBeDisabled()
  })
...
test.describe("VM Console", () => {
  ...
  test("console button opens console dialog when VM is running", async ({ createAndStartVM, page }) => {
    createdVmid = await createAndStartVM()
    ...
    await expect(page.getByTestId("vnc-console-dialog")).toBeVisible({ timeout: 10000 })
    ...
    await expect(page.getByTestId("vnc-console-dialog")).not.toBeVisible({ timeout: 5000 })
  })
})
